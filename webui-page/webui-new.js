...
function setTrackList(tracklist) {
  ...
  document.querySelector(".subtitle-track .detail-value").innerText = window.subs.selected + '/' + window.subs.count;
  document.querySelector(".audio-track .detail-value").innerText = window.audios.selected + '/' + window.audios.count;
}
...
function touchRangeOffset(e) {
  if (!e.layerX) {
    ...
    document.getElementById("volume").removeEventListener('touchstart', handleVolumeStart, false);
    document.getElementById("volume").removeEventListener('touchmove', handleVolumeMove, false);
    document.getElementById("volume").removeEventListener('touchend', handleVolumeEnd, false);
    return false
  }
  e.preventDefault();
...
function setPosSlider(position, duration) {
  ...
  const pos = document.querySelector(".current-time");
  slider.max = duration;
...
function handleMediaPositionStart(e) {
  console.log("moving media position");
...
function handleVolumeStart(e) {
  const offset = touchRangeOffset(e);
  setVolumeSlider(offset, document.getElementById("volume").max);
}
...
document.getElementById("volume").addEventListener('touchstart', handleVolumeStart, false);
document.getElementById("volume").addEventListener('touchmove', handleVolumeMove, false);
document.getElementById("volume").addEventListener('touchend', handleVolumeEnd, false);
...
function handleStatusResponse(json) {
  ...
  document.querySelector('.speed .detail-value').innerHTML =
    json['speed'].toFixed(2);

  document.querySelector(".subtitle-delay .detail-value").innerHTML =
    json['sub-delay'];
